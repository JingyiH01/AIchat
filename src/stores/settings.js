...
export const useSettingsStore = defineStore('settings', {
    ...
    state: () => ({
        ...
        maxTokens: 1000,
        // 当前使用的AI模型名称，默认为 'THUDM/glm-4-9b-chat'
        model: 'THUDM/glm-4-9b-chat',
        // 访问AI接口的API 密钥，默认为空字符串
        apiKey: '',
        // 是否启用流式响应，默认为 true
        streamResponse: true,
...
export const modelOptions = [
  //lable：显示在界面上的模型名称（用户友好名称）
  //value：模型的唯一标识（API调用时使用的名称）。该值会被传递到后端API，用于指定实际调用的模型。
  ...
  // VLM 模型
  //isVLM，仅VLM模型包含该字段且值为true，用于前端判断模型是否支持图像识别。
  { label: 'Qwen3-VL-8B-Thinking(支持图像)', value: 'Qwen/Qwen3-VL-8B-Thinking', isVLM: true },
]